...
export function registerBrandTools(server: McpServer) {
  server.tool('list_brands', 'List all brands with their status and mode', {}, async () => {
  ...
  server.tool(
    ...
    'Update brand settings: mode, modules, guardrails, active status',
    {
      brandId: z.string().uuid().describe('Brand UUID'),
      mode: z
        .enum(['observe', 'assist', 'auto'])
        .optional()
        .describe(
          'Operating mode: observe (monitor only), assist (drafts require approval), auto (safe actions auto-execute)',
        ),
