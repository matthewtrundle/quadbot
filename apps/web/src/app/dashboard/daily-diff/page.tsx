...
export default async function DailyDiffPage() {
  ...
  try {
    ...
    const wins = await db
      ...
      .where(
        brandFilter ? and(gte(outcomes.measured_at, yesterday), brandFilter) : gte(outcomes.measured_at, yesterday),
      )
    ...
    console.error('Daily diff query failed:', err);
    return (
      <div className="flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
        <AlertTriangle className="h-4 w-4 flex-shrink-0" />
        <p>Failed to load daily diff. Please try refreshing.</p>
      </div>
    );
  }
}
